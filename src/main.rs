...
// Пакет данных, который прилетает при регистрации/входе
#[derive(Deserialize)]
struct AuthRequest {
    login: String,
    password_hash: String, // Клиент пришлет обычный пароль в этом поле
}
...
async fn main() {
    ...
    // Добавляем новые HTTP-маршруты для регистрации и входа
    let app = Router::new()
...
async fn register_handler(
    ...
    // Хэшируем пароль с помощью bcrypt перед записью в базу (в целях безопасности)
    ...
    match db.execute(
        ...
        Err(e) => {
            // Если такой логин уже есть, SQLite вернет ошибку уникальности
...
async fn login_handler(
    ...
    match user_row {
        Ok((user_id, saved_hash)) => {
            // Проверяем, подходит ли введенный пароль к хэшу из базы
